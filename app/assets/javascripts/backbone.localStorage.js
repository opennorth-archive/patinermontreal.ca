...
window.Store = function(name) {
  this.name = name;
  var storage = store.get(this.name);
...
Backbone.localSync = function(method, model, options, error) {
  ...
  var resp;
  var storage = model.localStorage || model.collection.localStorage;

  switch (method) {
    case "read":    resp = model.id != undefined ? storage.find(model) : storage.findAll(); break;
    case "create":  resp = storage.create(model);                            break;
    case "update":  resp = storage.update(model);                            break;
    case "delete":  resp = storage.destroy(model);                           break;
  }
